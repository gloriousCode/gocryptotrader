...
func (e *Exchange) ExecuteOrder(o order.Event, dh data.Handler, om *engine.OrderManager, funds funding.IFundReleaser) (fill.Event, error) {
	...
	if cs.UseRealOrders {
		...
		f.Slippage = slippageRate.Mul(udecimal.MustFromInt64(100, 0)).Sub(udecimal.MustFromInt64(100, 0))
	}
...
func summarisePosition(direction gctorder.Side, orderAmount, orderTotal, orderFee udecimal.Decimal, pair, underlying currency.Pair) string {
	...
	return fmt.Sprintf("Placed %s order of %v %v for %v %v, with %v %v in fees, totalling %v %v",
		direction,
		orderAmount.RoundBank(8),
		baseCurr,
		orderTotal.RoundBank(8),
		quoteCurr,
		orderFee.RoundBank(8),
		quoteCurr,
		orderTotal.Add(orderFee).RoundBank(8),
		quoteCurr,
	)
}
...
func reduceAmountToFitPortfolioLimit(adjustedPrice, amount, sizedPortfolioTotal udecimal.Decimal, side gctorder.Side) udecimal.Decimal {
	...
	case gctorder.Buy, gctorder.Bid:
		if adjustedPrice.Mul(amount).GreaterThan(sizedPortfolioTotal) {
			...
			amount, _ = sizedPortfolioTotal.Div(adjustedPrice)
		}
	case gctorder.Sell, gctorder.Ask:
...
func applySlippageToPrice(direction gctorder.Side, price, slippageRate udecimal.Decimal) (udecimal.Decimal, error) {
	...
	switch direction {
	case gctorder.Buy, gctorder.Bid, gctorder.Long:
		adjustedPrice = price.Add(price.Mul(udecimal.MustFromInt64(1, 0).Sub(slippageRate)))
	case gctorder.Sell, gctorder.Ask, gctorder.Short:
