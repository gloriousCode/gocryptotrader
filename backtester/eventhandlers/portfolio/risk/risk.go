...
func (r *Risk) EvaluateOrder(o order.Event, latestHoldings []holdings.Holding, s compliance.Snapshot) (*order.Order, error) {
	...
	if len(latestHoldings) > 1 {
		...
		if lookup.MaximumHoldingRatio.GreaterThan(udecimal.Zero) && !ratio.Equal(udecimal.MustFromInt64(1, 0)) && ratio.GreaterThan(lookup.MaximumHoldingRatio) {
...
func existingLeverageRatio(s compliance.Snapshot) udecimal.Decimal {
	...
	for o := range s.Orders {
		if s.Orders[o].Order.Leverage != 0 {
			ordersWithLeverage = ordersWithLeverage.Add(udecimal.MustFromInt64(1, 0))
		}
	}
	result, _ := ordersWithLeverage.Div(udecimal.MustFromInt64(int64(len(s.Orders)), 0))
	return result
}
...
func assessHoldingsRatio(c currency.Pair, h []holdings.Holding) udecimal.Decimal {
	...
	ratio, _ := resp[c].Div(totalPosition)
