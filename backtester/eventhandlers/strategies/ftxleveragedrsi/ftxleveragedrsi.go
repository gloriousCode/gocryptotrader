...
const (
	...
	rsiLowKey           = "rsi-low"
	rsiHighKey          = "rsi-high"
	leverageOverrideKey = "leverage-override"
	description         = `The relative strength index is a technical indicator used in the analysis of financial markets. How fun would it be, to be so certain of its potential, that you would increase the size of your position using leverage?`
)
...
func (s *Strategy) OnSimultaneousSignals(d []data.Handler, _ funding.IFundingTransferer, _ portfolio.Handler) ([]signal.Event, error) {
	// what I want to know, is how much leverage is allowed,
	// then, what my leverage is through collateral
	// should a strategy even know the size? This should just raise an event with maximum allowable leverage
	// and the custom field allowing an override, so that a live strategy can then change it via GRPC. add in a mutex
