...
func TestLoadStrategyByName(t *testing.T) {
	...
	if resp.GetName() != dollarcostaverage.Name {
	...
	resp, err = LoadStrategyByName(technicalanalysis.Name, false)
	if !errors.Is(err, nil) {
		t.Errorf("received: %v, expected: %v", err, nil)
	}
	if resp.GetName() != technicalanalysis.Name {
		t.Error("expected rsi")
	}
	_, err = LoadStrategyByName(technicalanalysis.Name, true)
	if !errors.Is(err, nil) {
		t.Errorf("received: %v, expected: %v", err, nil)
	}
}
...
func (s *customStrategy) OnSimultaneousSignals([]data.Handler, funding.IFundingTransferer, portfolio.Handler) ([]signal.Event, error) {
	return nil, nil
}
