package math

import (
	"math"
	"testing"

	"github.com/quagmt/udecimal"
	"github.com/stretchr/testify/assert"
	"github.com/stretchr/testify/require"
)

func TestCalculateFee(t *testing.T) {
	t.Parallel()
	originalInput := float64(1)
	fee := float64(1)
	if expectedOutput, actualResult := 0.01, CalculateFee(originalInput, fee); expectedOutput != actualResult {
		t.Errorf(
			"Expected '%f'. Actual '%f'.", expectedOutput, actualResult)
	}
}

func TestCalculateAmountWithFee(t *testing.T) {
	t.Parallel()
	originalInput := float64(1)
	fee := float64(1)
	if actualResult, expectedOutput := CalculateAmountWithFee(originalInput, fee), 1.01; expectedOutput != actualResult {
		t.Errorf(
			"Expected '%f'. Actual '%f'.", expectedOutput, actualResult)
	}
}

func TestPercentageChange(t *testing.T) {
	t.Parallel()
	assert.Equal(t, 3.3333333333333335, PercentageChange(9000, 9300))
	assert.Equal(t, -3.225806451612903, PercentageChange(9300, 9000))
	assert.True(t, math.IsNaN(PercentageChange(0, 0)))
	assert.Equal(t, 0.0, PercentageChange(1, 1))
	assert.Equal(t, 0.0, PercentageChange(-1, -1))
	assert.True(t, math.IsInf(PercentageChange(0, 1), 1))
	assert.Equal(t, -100., PercentageChange(1, 0))
}

func TestPercentageDifference(t *testing.T) {
	t.Parallel()
	require.Equal(t, 196.03960396039605, PercentageDifference(1, 100))
	require.Equal(t, 196.03960396039605, PercentageDifference(100, 1))
	require.Equal(t, 0.13605442176870758, PercentageDifference(1.469, 1.471))
	require.Equal(t, 0.13605442176870758, PercentageDifference(1.471, 1.469))
	require.Equal(t, 0.0, PercentageDifference(1.0, 1.0))
	require.True(t, math.IsNaN(PercentageDifference(0.0, 0.0)))
}

// 1000000000	         0.2215 ns/op	       0 B/op	       0 allocs/op
func BenchmarkPercentageDifference(b *testing.B) {
	for b.Loop() {
		PercentageDifference(1.469, 1.471)
	}
}

func TestPercentageDifferenceDecimal(t *testing.T) {
	t.Parallel()
	tc1 := PercentageDifferenceDecimal(one, udecimal.MustFromFloat64(100))
	assert.Equal(t, "196.0396039603960396", tc1.String())

	tc2 := PercentageDifferenceDecimal(udecimal.MustFromFloat64(100), one)
	assert.Equal(t, "196.0396039603960396", tc2.String())

	tc3 := PercentageDifferenceDecimal(udecimal.MustFromFloat64(1.469), udecimal.MustFromFloat64(1.471))
	assert.Equal(t, "0.13605442176870748", tc3.String())

	tc4 := PercentageDifferenceDecimal(udecimal.MustFromFloat64(1.471), udecimal.MustFromFloat64(1.469))
	assert.Equal(t, "0.13605442176870748", tc4.String())

	tc5 := PercentageDifferenceDecimal(udecimal.MustFromFloat64(1.0), udecimal.MustFromFloat64(1.0))
	assert.Equal(t, "0", tc5.String())

	tc6 := PercentageDifferenceDecimal(udecimal.Zero, udecimal.Zero)
	assert.Equal(t, "0", tc6.String())
}

// 21409010	       54.99 ns/op	       0 B/op	       0 allocs/op
func BenchmarkDecimalPercentageDifference(b *testing.B) {
	d1, d2 := udecimal.MustFromFloat64(1.469), udecimal.MustFromFloat64(1.471)
	for b.Loop() {
		_ = PercentageDifferenceDecimal(d1, d2)
	}
}

func TestCalculateNetProfit(t *testing.T) {
	t.Parallel()
	amount := float64(5)
	priceThen := float64(1)
	priceNow := float64(10)
	costs := float64(1)
	actualResult := CalculateNetProfit(amount, priceThen, priceNow, costs)
	if expectedOutput := float64(44); expectedOutput != actualResult {
		t.Errorf(
			"Expected '%f'. Actual '%f'.", expectedOutput, actualResult)
	}
}

func TestRoundFloat(t *testing.T) {
	t.Parallel()
	// mapping of input vs expected result : map[precision]map[testedValue]expectedOutput
	testTableValues := map[int]map[float64]float64{
		0: {
			2.23456789:  2,
			-2.23456789: -2,
		},
		1: {
			2.23456789:  2.2,
			-2.23456789: -2.2,
		},
		2: {
			2.23456789:  2.23,
			-2.23456789: -2.23,
		},
		4: {
			2.23456789:  2.2346,
			-2.23456789: -2.2346,
		},
		8: {
			2.23456781:  2.23456781,
			-2.23456781: -2.23456781,
		},
	}

	for precision, values := range testTableValues {
		for testInput, expectedOutput := range values {
			actualOutput := RoundFloat(testInput, precision)
			if actualOutput != expectedOutput {
				t.Errorf("RoundFloat Expected '%v'. Actual '%v' on precision %d",
					expectedOutput, actualOutput, precision)
			}
		}
	}
}

func TestSortinoRatio(t *testing.T) {
	t.Parallel()
	rfr := 0.001
	figures := []float64{0.10, 0.04, 0.15, -0.05, 0.20, -0.02, 0.08, -0.06, 0.13, 0.23}
	avg, err := ArithmeticMean(figures)
	if err != nil {
		t.Error(err)
	}
	_, err = SortinoRatio(nil, rfr, avg)
	assert.ErrorIs(t, err, errZeroValue)

	var r float64
	r, err = SortinoRatio(figures, rfr, avg)
	if err != nil {
		t.Error(err)
	}
	if r != 3.0377875479459906 {
		t.Errorf("expected 3.0377875479459906, received %v", r)
	}
	avg, err = FinancialGeometricMean(figures)
	if err != nil {
		t.Error(err)
	}
	r, err = SortinoRatio(figures, rfr, avg)
	if err != nil {
		t.Error(err)
	}
	if r != 2.8712802265603243 {
		t.Errorf("expected 2.525203164136098, received %v", r)
	}

	// this follows and matches the example calculation from
	// https://www.wallstreetmojo.com/sortino-ratio/
	example := []float64{
		0.1,
		0.12,
		0.07,
		-0.03,
		0.08,
		-0.04,
		0.15,
		0.2,
		0.12,
		0.06,
		-0.03,
		0.02,
	}
	avg, err = ArithmeticMean(example)
	if err != nil {
		t.Error(err)
	}
	r, err = SortinoRatio(example, 0.06, avg)
	if err != nil {
		t.Error(err)
	}
	if rr := math.Round(r*10) / 10; rr != 0.2 {
		t.Errorf("expected 0.2, received %v", rr)
	}
}

func TestInformationRatio(t *testing.T) {
	t.Parallel()
	figures := []float64{0.0665, 0.0283, 0.0911, 0.0008, -0.0203, -0.0978, 0.0164, -0.0537, 0.078, 0.0032, 0.0249, 0}
	comparisonFigures := []float64{0.0216, 0.0048, 0.036, 0.0303, 0.0043, -0.0694, 0.0179, -0.0918, 0.0787, 0.0297, 0.003, 0}
	avg, err := ArithmeticMean(figures)
	if err != nil {
		t.Error(err)
	}
	if avg != 0.01145 {
		t.Error(avg)
	}
	var avgComparison float64
	avgComparison, err = ArithmeticMean(comparisonFigures)
	if err != nil {
		t.Error(err)
	}
	if avgComparison != 0.005425 {
		t.Error(avgComparison)
	}

	eachDiff := make([]float64, len(figures))
	for i := range figures {
		eachDiff[i] = figures[i] - comparisonFigures[i]
	}
	stdDev, err := PopulationStandardDeviation(eachDiff)
	if err != nil {
		t.Error(err)
	}
	if stdDev != 0.028992588851865803 {
		t.Error(stdDev)
	}
	information := (avg - avgComparison) / stdDev
	if information != 0.20781172839666107 {
		t.Errorf("expected %v received %v", 0.20781172839666107, information)
	}
	var information2 float64
	information2, err = InformationRatio(figures, comparisonFigures, avg, avgComparison)
	if err != nil {
		t.Error(err)
	}
	if information != information2 {
		t.Error(information2)
	}

	_, err = InformationRatio(figures, []float64{1}, avg, avgComparison)
	assert.ErrorIs(t, err, errInformationBadLength)
}

func TestCalmarRatio(t *testing.T) {
	t.Parallel()
	_, err := CalmarRatio(0, 0, 0, 0)
	assert.ErrorIs(t, err, errCalmarHighest)

	var ratio float64
	ratio, err = CalmarRatio(50000, 15000, 0.2, 0.1)
	if err != nil {
		t.Error(err)
	}
	if ratio != 0.14285714285714288 {
		t.Error(ratio)
	}
}

func TestCAGR(t *testing.T) {
	t.Parallel()
	_, err := CompoundAnnualGrowthRate(
		0,
		0,
		0,
		0)
	assert.ErrorIs(t, err, errCAGRNoIntervals)

	_, err = CompoundAnnualGrowthRate(
		0,
		0,
		0,
		1)
	assert.ErrorIs(t, err, errCAGRZeroOpenValue)

	var cagr float64
	cagr, err = CompoundAnnualGrowthRate(
		100,
		147,
		1,
		1)
	if err != nil {
		t.Error(err)
	}
	if cagr != 47 {
		t.Error("expected 47%")
	}
	cagr, err = CompoundAnnualGrowthRate(
		100,
		147,
		365,
		365)
	if err != nil {
		t.Error(err)
	}
	if cagr != 47 {
		t.Error("expected 47%")
	}

	cagr, err = CompoundAnnualGrowthRate(
		100,
		200,
		1,
		20)
	if err != nil {
		t.Error(err)
	}
	if cagr != 3.5264923841377582 {
		t.Error("expected 3.53%")
	}
}

func TestCalculateSharpeRatio(t *testing.T) {
	t.Parallel()
	result, err := SharpeRatio(nil, 0, 0)
	assert.ErrorIs(t, err, errZeroValue)

	if result != 0 {
		t.Error("expected 0")
	}

	result, err = SharpeRatio([]float64{0.026}, 0.017, 0.026)
	if err != nil {
		t.Error(err)
	}
	if result != 0 {
		t.Error("expected 0")
	}

	// this follows and matches the example calculation (without rounding) from
	// https://www.educba.com/sharpe-ratio-formula/
	returns := []float64{
		-0.0005,
		-0.0065,
		-0.0113,
		0.0031,
		-0.0112,
		0.0056,
		0.0156,
		0.0048,
		0.0012,
		0.0038,
		-0.0008,
		0.0032,
		0,
		-0.0128,
		-0.0058,
		0.003,
		0.0042,
		0.0055,
		0.0009,
	}
	var avg float64
	avg, err = ArithmeticMean(returns)
	if err != nil {
		t.Error(err)
	}
	result, err = SharpeRatio(returns, -0.0017, avg)
	if err != nil {
		t.Error(err)
	}
	result = math.Round(result*100) / 100
	if result != 0.26 {
		t.Errorf("expected 0.26, received %v", result)
	}
}

func TestStandardDeviation2(t *testing.T) {
	t.Parallel()
	r := []float64{9, 2, 5, 4, 12, 7}
	mean, err := ArithmeticMean(r)
	if err != nil {
		t.Error(err)
	}
	superMean := []float64{}
	for i := range r {
		result := math.Pow(r[i]-mean, 2)
		superMean = append(superMean, result)
	}
	superMeany := (superMean[0] + superMean[1] + superMean[2] + superMean[3] + superMean[4] + superMean[5]) / 5
	manualCalculation := math.Sqrt(superMeany)
	var codeCalcu float64
	codeCalcu, err = SampleStandardDeviation(r)
	if err != nil {
		t.Error(err)
	}
	if manualCalculation != codeCalcu && codeCalcu != 3.619 {
		t.Error("expected 3.619")
	}
}

func TestGeometricAverage(t *testing.T) {
	t.Parallel()
	values := []float64{1, 2, 3, 4, 5, 6, 7, 8}
	_, err := GeometricMean(nil)
	assert.ErrorIs(t, err, errZeroValue)

	var mean float64
	mean, err = GeometricMean(values)
	if err != nil {
		t.Error(err)
	}
	expected := 3.764350599503129
	if mean != expected {
		t.Errorf("expected %v, received %v", expected, mean)
	}

	values = []float64{15, 12, 13, 19, 10}
	mean, err = GeometricMean(values)
	if err != nil {
		t.Error(err)
	}
	expected2 := 13.477020583645698
	if math.Abs(mean-expected2) > 1e-10 {
		t.Errorf("expected %v, received %v", 13.50, mean)
	}

	values = []float64{-1, 12, 13, 19, 10}
	mean, err = GeometricMean(values)
	assert.ErrorIs(t, err, errGeometricNegative)

	if mean != 0 {
		t.Errorf("expected %v, received %v", 0, mean)
	}
}

func TestFinancialGeometricAverage(t *testing.T) {
	t.Parallel()
	values := []float64{1, 2, 3, 4, 5, 6, 7, 8}
	_, err := FinancialGeometricMean(nil)
	assert.ErrorIs(t, err, errZeroValue)

	var mean float64
	mean, err = FinancialGeometricMean(values)
	if err != nil {
		t.Error(err)
	}
	if mean != 3.9541639996482028 {
		t.Errorf("expected %v, received %v", 3.95, mean)
	}

	values = []float64{15, 12, 13, 19, 10}
	mean, err = FinancialGeometricMean(values)
	if err != nil {
		t.Error(err)
	}
	if mean != 13.49849123325646 {
		t.Errorf("expected %v, received %v", 13.50, mean)
	}

	values = []float64{-1, 12, 13, 19, 10}
	mean, err = FinancialGeometricMean(values)
	if err != nil {
		t.Error(err)
	}
	if mean != 0 {
		t.Errorf("expected %v, received %v", 0, mean)
	}

	values = []float64{-2, 12, 13, 19, 10}
	_, err = FinancialGeometricMean(values)
	assert.ErrorIs(t, err, errNegativeValueOutOfRange)
}

func TestArithmeticAverage(t *testing.T) {
	t.Parallel()
	values := []float64{1, 2, 3, 4, 5, 6, 7, 8}
	_, err := ArithmeticMean(nil)
	assert.ErrorIs(t, err, errZeroValue)

	var avg float64
	avg, err = ArithmeticMean(values)
	if err != nil {
		t.Error(err)
	}
	if avg != 4.5 {
		t.Error("expected 4.5")
	}
}

func TestDecimalSortinoRatio(t *testing.T) {
	t.Parallel()
	rfr := udecimal.MustFromFloat64(0.001)
	figures := []udecimal.Decimal{
		udecimal.MustFromFloat64(0.10),
		udecimal.MustFromFloat64(0.04),
		udecimal.MustFromFloat64(0.15),
		udecimal.MustFromFloat64(-0.05),
		udecimal.MustFromFloat64(0.20),
		udecimal.MustFromFloat64(-0.02),
		udecimal.MustFromFloat64(0.08),
		udecimal.MustFromFloat64(-0.06),
		udecimal.MustFromFloat64(0.13),
		udecimal.MustFromFloat64(0.23),
	}
	avg, err := DecimalArithmeticMean(figures)
	require.NoError(t, err)
	_, err = DecimalSortinoRatio(nil, rfr, avg)
	assert.ErrorIs(t, err, errZeroValue)

	r, err := DecimalSortinoRatio(figures, rfr, avg)
	rf := r.InexactFloat64()
	assert.Equal(t, 3.0377875479459906, rf)

	avg, err = DecimalFinancialGeometricMean(figures)
	require.NoError(t, err)
	r, err = DecimalSortinoRatio(figures, rfr, avg)
	assert.True(t, r.Equal(udecimal.MustFromFloat64(2.8712802265603243204)))

	// this follows and matches the example calculation from
	// https://www.wallstreetmojo.com/sortino-ratio/
	example := []udecimal.Decimal{
		udecimal.MustFromFloat64(0.1),
		udecimal.MustFromFloat64(0.12),
		udecimal.MustFromFloat64(0.07),
		udecimal.MustFromFloat64(-0.03),
		udecimal.MustFromFloat64(0.08),
		udecimal.MustFromFloat64(-0.04),
		udecimal.MustFromFloat64(0.15),
		udecimal.MustFromFloat64(0.2),
		udecimal.MustFromFloat64(0.12),
		udecimal.MustFromFloat64(0.06),
		udecimal.MustFromFloat64(-0.03),
		udecimal.MustFromFloat64(0.02),
	}
	avg, err = DecimalArithmeticMean(example)
	require.NoError(t, err)
	r, err = DecimalSortinoRatio(example, udecimal.MustFromFloat64(0.06), avg)
	assert.True(t, r.RoundHTZ(1).Equal(udecimal.MustFromFloat64(0.2)))
}

func TestDecimalInformationRatio(t *testing.T) {
	t.Parallel()
	figures := []udecimal.Decimal{
		udecimal.MustFromFloat64(0.0665),
		udecimal.MustFromFloat64(0.0283),
		udecimal.MustFromFloat64(0.0911),
		udecimal.MustFromFloat64(0.0008),
		udecimal.MustFromFloat64(-0.0203),
		udecimal.MustFromFloat64(-0.0978),
		udecimal.MustFromFloat64(0.0164),
		udecimal.MustFromFloat64(-0.0537),
		udecimal.MustFromFloat64(0.078),
		udecimal.MustFromFloat64(0.0032),
		udecimal.MustFromFloat64(0.0249),
		udecimal.Zero,
	}
	comparisonFigures := []udecimal.Decimal{
		udecimal.MustFromFloat64(0.0216),
		udecimal.MustFromFloat64(0.0048),
		udecimal.MustFromFloat64(0.036),
		udecimal.MustFromFloat64(0.0303),
		udecimal.MustFromFloat64(0.0043),
		udecimal.MustFromFloat64(-0.0694),
		udecimal.MustFromFloat64(0.0179),
		udecimal.MustFromFloat64(-0.0918),
		udecimal.MustFromFloat64(0.0787),
		udecimal.MustFromFloat64(0.0297),
		udecimal.MustFromFloat64(0.003),
		udecimal.Zero,
	}
	avg, err := DecimalArithmeticMean(figures)
	require.NoError(t, err)
	assert.True(t, udecimal.MustFromFloat64(0.01145).Equal(avg))

	avgComparison, err := DecimalArithmeticMean(comparisonFigures)
	require.NoError(t, err)
	assert.True(t, udecimal.MustFromFloat64(0.005425).Equal(avgComparison))

	eachDiff := make([]udecimal.Decimal, len(figures))
	for i := range figures {
		eachDiff[i] = figures[i].Sub(comparisonFigures[i])
	}
	stdDev, err := DecimalPopulationStandardDeviation(eachDiff)
	assert.Equal(t, udecimal.MustFromFloat64(0.028992588851865803), stdDev)

	_, err = avg.Sub(avgComparison).Div(stdDev)
	require.NoError(t, err)

	information2, err := DecimalInformationRatio(figures, comparisonFigures, avg, avgComparison)
	require.NoError(t, err)
	t.Logf("Actual information2: %v", information2)
	assert.Equal(t, udecimal.MustFromFloat64(0.2078117283966610747), information2)

	_, err = DecimalInformationRatio(figures, []udecimal.Decimal{one}, avg, avgComparison)
	assert.ErrorIs(t, err, errInformationBadLength)
}

func TestDecimalCalmarRatio(t *testing.T) {
	t.Parallel()
	_, err := DecimalCalmarRatio(udecimal.Zero, udecimal.Zero, udecimal.Zero, udecimal.Zero)
	assert.ErrorIs(t, err, errCalmarHighest)

	var ratio udecimal.Decimal
	ratio, err = DecimalCalmarRatio(
		udecimal.MustFromFloat64(50000),
		udecimal.MustFromFloat64(15000),
		udecimal.MustFromFloat64(0.2),
		udecimal.MustFromFloat64(0.1))
	if err != nil {
		t.Error(err)
	}
	if !ratio.Equal(udecimal.MustFromFloat64(0.1428571428571428571)) {
		t.Error(ratio)
	}
}

func TestDecimalCalculateSharpeRatio(t *testing.T) {
	t.Parallel()
	result, err := DecimalSharpeRatio(nil, udecimal.Zero, udecimal.Zero)
	assert.ErrorIs(t, err, errZeroValue)

	if !result.IsZero() {
		t.Error("expected 0")
	}

	result, err = DecimalSharpeRatio([]udecimal.Decimal{udecimal.MustFromFloat64(0.026)}, udecimal.MustFromFloat64(0.017), udecimal.MustFromFloat64(0.026))
	if err != nil {
		t.Error(err)
	}
	if !result.IsZero() {
		t.Error("expected 0")
	}

	// this follows and matches the example calculation (without rounding) from
	// https://www.educba.com/sharpe-ratio-formula/
	returns := []udecimal.Decimal{
		udecimal.MustFromFloat64(-0.0005),
		udecimal.MustFromFloat64(-0.0065),
		udecimal.MustFromFloat64(-0.0113),
		udecimal.MustFromFloat64(0.0031),
		udecimal.MustFromFloat64(-0.0112),
		udecimal.MustFromFloat64(0.0056),
		udecimal.MustFromFloat64(0.0156),
		udecimal.MustFromFloat64(0.0048),
		udecimal.MustFromFloat64(0.0012),
		udecimal.MustFromFloat64(0.0038),
		udecimal.MustFromFloat64(-0.0008),
		udecimal.MustFromFloat64(0.0032),
		udecimal.Zero,
		udecimal.MustFromFloat64(-0.0128),
		udecimal.MustFromFloat64(-0.0058),
		udecimal.MustFromFloat64(0.003),
		udecimal.MustFromFloat64(0.0042),
		udecimal.MustFromFloat64(0.0055),
		udecimal.MustFromFloat64(0.0009),
	}
	var avg udecimal.Decimal
	avg, err = DecimalArithmeticMean(returns)
	if err != nil {
		t.Error(err)
	}
	result, err = DecimalSharpeRatio(returns, udecimal.MustFromFloat64(-0.0017), avg)
	if err != nil {
		t.Error(err)
	}
	result = result.RoundHTZ(2)
	if !result.Equal(udecimal.MustFromFloat64(0.26)) {
		t.Errorf("expected 0.26, received %v", result)
	}
}

func TestDecimalStandardDeviation2(t *testing.T) {
	t.Parallel()
	r := []udecimal.Decimal{
		udecimal.MustFromFloat64(9),
		udecimal.MustFromFloat64(2),
		udecimal.MustFromFloat64(5),
		udecimal.MustFromFloat64(4),
		udecimal.MustFromFloat64(12),
		udecimal.MustFromFloat64(7),
	}
	mean, err := DecimalArithmeticMean(r)
	if err != nil {
		t.Error(err)
	}
	superMean := make([]udecimal.Decimal, len(r))
	for i := range r {
		result, err := r[i].Sub(mean).PowToIntPart(udecimal.MustFromFloat64(2))
		require.NoError(t, err)
		superMean[i] = result
	}
	superMeany, err := superMean[0].Add(superMean[1].Add(superMean[2].Add(superMean[3].Add(superMean[4].Add(superMean[5]))))).Div(udecimal.MustFromFloat64(5))
	require.NoError(t, err)
	manualCalculation := udecimal.MustFromFloat64(math.Sqrt(superMeany.InexactFloat64()))
	var codeCalcu udecimal.Decimal
	codeCalcu, err = DecimalSampleStandardDeviation(r)
	if err != nil {
		t.Error(err)
	}
	if !manualCalculation.Equal(codeCalcu) && codeCalcu.Equal(udecimal.MustFromFloat64(3.619)) {
		t.Error("expected 3.619")
	}
}

func TestDecimalGeometricAverage(t *testing.T) {
	t.Parallel()
	values := []udecimal.Decimal{
		udecimal.MustFromFloat64(1),
		udecimal.MustFromFloat64(2),
		udecimal.MustFromFloat64(3),
		udecimal.MustFromFloat64(4),
		udecimal.MustFromFloat64(5),
		udecimal.MustFromFloat64(6),
		udecimal.MustFromFloat64(7),
		udecimal.MustFromFloat64(8),
	}
	_, err := DecimalGeometricMean(nil)
	assert.ErrorIs(t, err, errZeroValue)

	var mean udecimal.Decimal
	mean, err = DecimalGeometricMean(values)
	if err != nil {
		t.Error(err)
	}
	expected := udecimal.MustFromFloat64(3.764350599503129)
	if mean.Sub(expected).Abs().GreaterThan(udecimal.MustFromFloat64(1e-10)) {
		t.Errorf("expected %v, received %v", 3.76, mean)
	}

	values = []udecimal.Decimal{
		udecimal.MustFromFloat64(15),
		udecimal.MustFromFloat64(12),
		udecimal.MustFromFloat64(13),
		udecimal.MustFromFloat64(19),
		udecimal.MustFromFloat64(10),
	}
	mean, err = DecimalGeometricMean(values)
	if err != nil {
		t.Error(err)
	}
	expected2 := udecimal.MustFromFloat64(13.477020583645698)
	if mean.Sub(expected2).Abs().GreaterThan(udecimal.MustFromFloat64(1e-10)) {
		t.Errorf("expected %v, received %v", 13.50, mean)
	}

	values = []udecimal.Decimal{
		udecimal.MustFromFloat64(-1),
		udecimal.MustFromFloat64(12),
		udecimal.MustFromFloat64(13),
		udecimal.MustFromFloat64(19),
		udecimal.MustFromFloat64(10),
	}
	mean, err = DecimalGeometricMean(values)
	assert.ErrorIs(t, err, errGeometricNegative)

	if !mean.IsZero() {
		t.Errorf("expected %v, received %v", 0, mean)
	}
}

func TestDecimalFinancialGeometricAverage(t *testing.T) {
	t.Parallel()
	values := []udecimal.Decimal{
		udecimal.MustFromFloat64(1),
		udecimal.MustFromFloat64(2),
		udecimal.MustFromFloat64(3),
		udecimal.MustFromFloat64(4),
		udecimal.MustFromFloat64(5),
		udecimal.MustFromFloat64(6),
		udecimal.MustFromFloat64(7),
		udecimal.MustFromFloat64(8),
	}
	_, err := DecimalFinancialGeometricMean(nil)
	assert.ErrorIs(t, err, errZeroValue)

	var mean udecimal.Decimal
	mean, err = DecimalFinancialGeometricMean(values)
	if err != nil {
		t.Error(err)
	}
	if !mean.Equal(udecimal.MustFromFloat64(3.9541639996482028)) {
		t.Errorf("expected %v, received %v", 3.95, mean)
	}

	values = []udecimal.Decimal{
		udecimal.MustFromFloat64(15),
		udecimal.MustFromFloat64(12),
		udecimal.MustFromFloat64(13),
		udecimal.MustFromFloat64(19),
		udecimal.MustFromFloat64(10),
	}
	mean, err = DecimalFinancialGeometricMean(values)
	if err != nil {
		t.Error(err)
	}
	if !mean.Equal(udecimal.MustFromFloat64(13.49849123325646)) {
		t.Errorf("expected %v, received %v", 13.50, mean)
	}

	values = []udecimal.Decimal{
		udecimal.MustFromFloat64(-1),
		udecimal.MustFromFloat64(12),
		udecimal.MustFromFloat64(13),
		udecimal.MustFromFloat64(19),
		udecimal.MustFromFloat64(10),
	}
	mean, err = DecimalFinancialGeometricMean(values)
	if err != nil {
		t.Error(err)
	}
	if !mean.IsZero() {
		t.Errorf("expected %v, received %v", 0, mean)
	}

	values = []udecimal.Decimal{
		udecimal.MustFromFloat64(-2),
		udecimal.MustFromFloat64(12),
		udecimal.MustFromFloat64(13),
		udecimal.MustFromFloat64(19),
		udecimal.MustFromFloat64(10),
	}
	_, err = DecimalFinancialGeometricMean(values)
	assert.ErrorIs(t, err, errNegativeValueOutOfRange)
}

func TestDecimalArithmeticAverage(t *testing.T) {
	t.Parallel()
	values := []udecimal.Decimal{
		udecimal.MustFromFloat64(1),
		udecimal.MustFromFloat64(2),
		udecimal.MustFromFloat64(3),
		udecimal.MustFromFloat64(4),
		udecimal.MustFromFloat64(5),
		udecimal.MustFromFloat64(6),
		udecimal.MustFromFloat64(7),
		udecimal.MustFromFloat64(8),
	}
	_, err := DecimalArithmeticMean(nil)
	assert.ErrorIs(t, err, errZeroValue)

	var avg udecimal.Decimal
	avg, err = DecimalArithmeticMean(values)
	if err != nil {
		t.Error(err)
	}
	if !avg.Equal(udecimal.MustFromFloat64(4.5)) {
		t.Error("expected 4.5")
	}
}
