...
func TestPercentageDifferenceDecimal(t *testing.T) {
	t.Parallel()
	require.Equal(t, "196.03960396039604", PercentageDifferenceDecimal(udecimal.MustFromFloat64(1), udecimal.MustFromFloat64(100)).String())
	require.Equal(t, "196.03960396039604", PercentageDifferenceDecimal(udecimal.MustFromFloat64(100), udecimal.MustFromFloat64(1)).String())
	require.Equal(t, "0.13605442176871", PercentageDifferenceDecimal(udecimal.MustFromFloat64(1.469), udecimal.MustFromFloat64(1.471)).String())
	require.Equal(t, "0.13605442176871", PercentageDifferenceDecimal(udecimal.MustFromFloat64(1.471), udecimal.MustFromFloat64(1.469)).String())
	require.Equal(t, "0", PercentageDifferenceDecimal(udecimal.MustFromFloat64(1.0), udecimal.MustFromFloat64(1.0)).String())
	require.Equal(t, "0", PercentageDifferenceDecimal(udecimal.Zero, udecimal.Zero).String())
}
...
func TestDecimalSortinoRatio(t *testing.T) {
	...
	r, err = DecimalSortinoRatio(figures, rfr, avg)
	assert.ErrorIs(t, err, ErrInexactConversion)
	assert.True(t, r.Equal(udecimal.MustFromFloat64(2.8712802265603243)))
	...
	assert.ErrorIs(t, err, ErrInexactConversion)
	assert.True(t, r.Round(1).Equal(udecimal.MustFromFloat64(0.2)))
}
...
func TestDecimalInformationRatio(t *testing.T) {
	...
	assert.Equal(t, udecimal.MustFromFloat64(0.028992588851865227), stdDev)

	information := avg.Sub(avgComparison).Div(stdDev)
	assert.Equal(t, udecimal.MustFromFloat64(0.2078117283966652), information)
	...
	_, err = DecimalInformationRatio(figures, []udecimal.Decimal{decimal.NewFromInt(1)}, avg, avgComparison)
	assert.ErrorIs(t, err, errInformationBadLength)
}
...
func TestDecimalCalmarRatio(t *testing.T) {
	...
	if !ratio.Equal(udecimal.MustFromFloat64(0.1428571428571429)) {
		t.Error(ratio)
	}
}
...
func TestDecimalGeometricAverage(t *testing.T) {
	...
	if !mean.Equal(udecimal.MustFromFloat64(3.764350599503129)) {
		t.Errorf("expected %v, received %v", 3.95, mean)
	}
	...
	if !mean.Equal(udecimal.MustFromFloat64(13.477020583645698)) {
		t.Errorf("expected %v, received %v", 13.50, mean)
	}
...
func TestDecimalPow(t *testing.T) {
	...
	// zero
	pow = DecimalPow(udecimal.Zero, decimal.NewFromInt(1))
	if !pow.Equal(udecimal.Zero) {
		t.Errorf("received '%v' expected '%v'", pow, 0)
	}

	// inf
	pow = DecimalPow(udecimal.Zero, decimal.NewFromInt(-3))
	if !pow.Equal(udecimal.Zero) {
		t.Errorf("received '%v' expected '%v'", pow, 0)
	}

	// nan
	pow = DecimalPow(decimal.NewFromInt(-1), udecimal.MustFromFloat64(0.1111))
	if !pow.Equal(udecimal.Zero) {
		t.Errorf("received '%v' expected '%v'", pow, 0)
	}
}
