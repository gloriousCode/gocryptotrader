...
func Series(exchangeName, curr string, asset string, start, end time.Time) (out RatesHolder, err error) {
	if exchangeName == "" || curr == "" || asset == "" {
		return out, errInvalidInput
	}

	queries := []qm.QueryMod{
		qm.Where("currency = ?", strings.ToUpper(curr)),
	...
	if repository.GetSQLDialect() == database.DBSQLite3 {
		...
		retCandle, errC := modelSQLite.FundingRates(queries...).All(context.TODO(), database.DB.SQL)
		if errC != nil {
			return out, errC
		}
	...
	if len(out.Rates) < 1 {
		return out, fmt.Errorf("%w: %s %s %s %v %s", ErrNoCandleDataFound, exchangeName, curr, interval, asset)
	}
...
func DeleteCandles(in *RatesHolder) (int64, error) {
	...
	if repository.GetSQLDialect() == database.DBSQLite3 {
		queries = append(queries, qm.Where("timestamp between ? and ?", in.Rates[0].Timestamp.UTC().Format(time.RFC3339), in.Rates[len(in.Rates)-1].Timestamp.UTC().Format(time.RFC3339)))
		return deleteSQLite(ctx, queries)
	}

	queries = append(queries, qm.Where("timestamp between ? and ?", in.Rates[0].Timestamp.UTC(), in.Rates[len(in.Rates)-1].Timestamp.UTC()))
...
func Insert(in *RatesHolder) (uint64, error) {
	if database.DB.SQL == nil {
		return 0, database.ErrDatabaseSupportDisabled
	}
...
func insertSQLite(ctx context.Context, tx *sql.Tx, in *RatesHolder) (uint64, error) {
	var totalInserted uint64
	for x := range in.Rates {
		var tempCandle = modelSQLite.Candle{
			...
			Timestamp:      in.Rates[x].Timestamp.UTC().Format(time.RFC3339),
			Open:           in.Rates[x].Open,
			High:           in.Rates[x].High,
			Low:            in.Rates[x].Low,
			Close:          in.Rates[x].Close,
			Volume:         in.Rates[x].Volume,
		}
		...
		tempCandle.ValidationJobID = null.String{String: in.Rates[x].ValidationJobID, Valid: in.Rates[x].ValidationJobID != ""}
		tempCandle.ValidationIssues = null.String{String: in.Rates[x].ValidationIssues, Valid: in.Rates[x].ValidationIssues != ""}
		tempCandle.SourceJobID = null.String{String: in.Rates[x].SourceJobID, Valid: in.Rates[x].SourceJobID != ""}
...
func insertPostgresSQL(ctx context.Context, tx *sql.Tx, in *RatesHolder) (uint64, error) {
	var totalInserted uint64
	for x := range in.Rates {
		var tempCandle = modelPSQL.Candle{
			...
			Asset:          strings.ToLower(in.Asset),
			Timestamp:      in.Rates[x].Timestamp,
			Open:           in.Rates[x].Open,
			High:           in.Rates[x].High,
			Low:            in.Rates[x].Low,
			Close:          in.Rates[x].Close,
...
func InsertFromCSV(exchangeName, curr string, interval int64, asset, file string) (uint64, error) {
	...
	for {
		...
		tempCandle.Rates = append(tempCandle.Rates, tempTick)
	}
	if err != nil {
		return 0, err
	}
