package fees

import (
	"errors"
	"fmt"
	"sync"
	"time"

	"github.com/thrasher-corp/gocryptotrader/common"
	"github.com/thrasher-corp/gocryptotrader/common/key"
)

var (
	ErrNoFeesLoaded = errors.New("no fees loaded")
	ErrNoFeeFound   = errors.New("no fee found")
	ErrInvalidFee   = errors.New("invalid fee configuration")

	errExchangeNameEmpty = errors.New("exchange name is empty")
	errAssetInvalid      = errors.New("asset is invalid")
	errPairNotSet        = errors.New("currency pair not set")
)

type store struct {
	epaFees map[*key.ExchangeAssetPair]*Fee
	mtx     sync.RWMutex
}

var manager = store{
	epaFees: make(map[*key.ExchangeAssetPair]*Fee),
}

type Fee struct {
	Key         *key.ExchangeAssetPair
	MakerFee    float64
	TakerFee    float64
	Tier        uint8
	VolumeBased bool
	UpdatedAt   time.Time
}

// Load loads all limits into private limit holder
func Load(fees []Fee) error {
	return manager.load(fees)
}

// GetFee returns the fee matching the key
func GetFee(k *key.ExchangeAssetPair) (*Fee, error) {
	return manager.getFee(k)
}

// EstimateFee is a convenience method to estimate fees without retrieving the fee structure first
func EstimateFee(k *key.ExchangeAssetPair, price, amount float64, isMaker bool) (float64, error) {
	return manager.estimateFee(k, price, amount, isMaker)
}

func (e *store) load(fees []Fee) error {
	if len(fees) == 0 {
		return common.ErrEmptyParams
	}
	e.mtx.Lock()
	defer e.mtx.Unlock()
	if e.epaFees == nil {
		e.epaFees = make(map[key.ExchangeAssetPair]*Fee)
	}

	for x := range fees {
		if fees[x].Key.Exchange == "" {
			return fmt.Errorf("cannot load fees for %q %q: %w", fees[x].Key.Asset, fees[x].Key.Pair(), errExchangeNameEmpty)
		}
		if !fees[x].Key.Asset.IsValid() {
			return fmt.Errorf("cannot load fees for %q %q: %w", fees[x].Key.Exchange, fees[x].Key.Pair(), errAssetInvalid)
		}
		if fees[x].Key.Pair().IsEmpty() {
			return fmt.Errorf("cannot load fees for %q %q: %w", fees[x].Key.Exchange, fees[x].Key.Asset, errPairNotSet)
		}
		if (fees[x].TakerFee == 0 && fees[x].MakerFee == 0) ||
			fees[x].TakerFee < fees[x].MakerFee {
			return fmt.Errorf("%w for %s",
				ErrInvalidFee,
				fees[x].Key)
		}
		fees[x].UpdatedAt = time.Now()
		e.epaFees[fees[x].Key] = &fees[x]
	}
	return nil
}

func (e *store) getFee(k *key.ExchangeAssetPair) (*Fee, error) {
	e.mtx.RLock()
	defer e.mtx.RUnlock()
	if e.epaFees == nil {
		return nil, ErrNoFeesLoaded
	}
	f, ok := e.epaFees[k]
	if !ok {
		return nil, fmt.Errorf("%w for %s", ErrNoFeeFound, k)
	}
	return f, nil
}

func (e *store) estimateFee(k *key.ExchangeAssetPair, price, amount float64, isMaker bool) (float64, error) {
	e.mtx.RLock()
	defer e.mtx.RUnlock()
	if e.epaFees == nil {
		return -1, ErrNoFeesLoaded
	}
	m1, ok := e.epaFees[k]
	if !ok {
		return -1, fmt.Errorf("%w for %s", ErrNoFeeFound, k)
	}
	feeRate := m1.TakerFee
	if isMaker {
		feeRate = m1.MakerFee
	}
	return price * amount * feeRate, nil
}
