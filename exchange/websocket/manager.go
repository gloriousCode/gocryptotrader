...
func (m *Manager) EnableAndConnectNoSubs(ctx context.Context) error {
	...
	for i := range m.connectionManager {
		...
		m.Wg.Add(1)
		// The reader outlives Connect, so keep context values without request cancellation.
		go m.Reader(context.WithoutCancel(ctx), conn, m.connectionManager[i].setup.Handler)
