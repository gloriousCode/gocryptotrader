...
func (b *Binance) UpdateOrderbook(ctx context.Context, p currency.Pair, assetType asset.Item) (*orderbook.Base, error) {
	...
	switch assetType {
	case asset.Spot, asset.Margin:
		orderbookNew, err = b.GetOrderBook(ctx, OrderBookDataRequestParams{p, 20})
	case asset.USDTMarginedFutures:
		orderbookNew, err = b.UFuturesOrderbook(ctx, p, 20)
	case asset.CoinMarginedFutures:
		orderbookNew, err = b.GetFuturesOrderbook(ctx, p, 50)
	default:
