...
func TestGetWalletBalance(t *testing.T) {
	...
	if mockTests {
		...
		assert.Equal(t, types.NewNumberFromFloat(0.1997), r.List[0].AccountIMRate, "AccountIMRate should match")
		assert.Equal(t, types.NewNumberFromFloat(0.4996), r.List[0].AccountLTV, "AccountLTV should match")
		assert.Equal(t, types.NewNumberFromFloat(0.0399), r.List[0].AccountMMRate, "AccountMMRate should match")
		assert.Equal(t, "UNIFIED", r.List[0].AccountType, "AccountType should match")
		assert.Equal(t, types.NewNumberFromFloat(24616.49915805), r.List[0].TotalAvailableBalance, "TotalAvailableBalance should match")
		assert.Equal(t, types.NewNumberFromFloat(41445.9203332), r.List[0].TotalEquity, "TotalEquity should match")
		assert.Equal(t, types.NewNumberFromFloat(6144.46796478), r.List[0].TotalInitialMargin, "TotalInitialMargin should match")
		assert.Equal(t, types.NewNumberFromFloat(1228.89359295), r.List[0].TotalMaintenanceMargin, "TotalMaintenanceMargin should match")
		assert.Equal(t, types.NewNumberFromFloat(30760.96712284), r.List[0].TotalMarginBalance, "TotalMarginBalance should match")
		assert.Equal(t, types.NewNumberFromFloat(0.0), r.List[0].TotalPerpUPL, "TotalPerpUPL should match")
		assert.Equal(t, types.NewNumberFromFloat(30760.96712284), r.List[0].TotalWalletBalance, "TotalWalletBalance should match")
		require.Len(t, r.List[0].Coin, 3, "GetWalletBalance should return 3 coins")

		for x := range r.List[0].Coin {
			switch x {
			case 0:
				assert.Equal(t, types.NewNumberFromFloat(0.21976631), r.List[0].Coin[x].AccruedInterest, "AccruedInterest should match")
				assert.Equal(t, types.NewNumberFromFloat(0), r.List[0].Coin[x].AvailableToBorrow, "AvailableToBorrow should match")
				assert.Equal(t, types.NewNumberFromFloat(0), r.List[0].Coin[x].AvailableToWithdraw, "AvailableToWithdraw should match")
				assert.Equal(t, types.NewNumberFromFloat(0), r.List[0].Coin[x].Bonus, "Bonus should match")
				assert.Equal(t, types.NewNumberFromFloat(30723.630216383711792744), r.List[0].Coin[x].BorrowAmount, "BorrowAmount should match")
				...
				assert.Equal(t, types.NewNumberFromFloat(0), r.List[0].Coin[x].CumulativeRealisedPNL, "CumulativeRealisedPNL should match")
				assert.Equal(t, types.NewNumberFromFloat(-30723.63021638), r.List[0].Coin[x].Equity, "Equity should match")
				assert.Equal(t, types.NewNumberFromFloat(0), r.List[0].Coin[x].Locked, "Locked should match")
				assert.True(t, r.List[0].Coin[x].MarginCollateral, "MarginCollateral should match")
				assert.Equal(t, types.NewNumberFromFloat(0), r.List[0].Coin[x].SpotHedgingQuantity, "SpotHedgingQuantity should match")
				assert.Equal(t, types.NewNumberFromFloat(0), r.List[0].Coin[x].TotalOrderIM, "TotalOrderIM should match")
				assert.Equal(t, types.NewNumberFromFloat(0), r.List[0].Coin[x].TotalPositionIM, "TotalPositionIM should match")
				assert.Equal(t, types.NewNumberFromFloat(0), r.List[0].Coin[x].TotalPositionMM, "TotalPositionMM should match")
				assert.Equal(t, types.NewNumberFromFloat(0), r.List[0].Coin[x].UnrealisedPNL, "UnrealisedPNL should match")
				assert.Equal(t, types.NewNumberFromFloat(-30722.33982391), r.List[0].Coin[x].USDValue, "USDValue should match")
				assert.Equal(t, types.NewNumberFromFloat(-30723.63021638), r.List[0].Coin[x].WalletBalance, "WalletBalance should match")
			case 1:
				assert.Equal(t, types.NewNumberFromFloat(0), r.List[0].Coin[x].AccruedInterest, "AccruedInterest should match")
				assert.Equal(t, types.NewNumberFromFloat(0), r.List[0].Coin[x].AvailableToBorrow, "AvailableToBorrow should match")
				assert.Equal(t, types.NewNumberFromFloat(1005.79191187), r.List[0].Coin[x].AvailableToWithdraw, "AvailableToWithdraw should match")
				assert.Equal(t, types.NewNumberFromFloat(0), r.List[0].Coin[x].Bonus, "Bonus should match")
				assert.Equal(t, types.NewNumberFromFloat(0), r.List[0].Coin[x].BorrowAmount, "BorrowAmount should match")
				assert.Equal(t, currency.AVAX, r.List[0].Coin[x].Coin, "Coin should match")
				assert.True(t, r.List[0].Coin[x].CollateralSwitch, "CollateralSwitch should match")
				assert.Equal(t, types.NewNumberFromFloat(0), r.List[0].Coin[x].CumulativeRealisedPNL, "CumulativeRealisedPNL should match")
				assert.Equal(t, types.NewNumberFromFloat(2473.9), r.List[0].Coin[x].Equity, "Equity should match")
				assert.Equal(t, types.NewNumberFromFloat(0), r.List[0].Coin[x].Locked, "Locked should match")
				assert.True(t, r.List[0].Coin[x].MarginCollateral, "MarginCollateral should match")
				assert.Equal(t, types.NewNumberFromFloat(0), r.List[0].Coin[x].SpotHedgingQuantity, "SpotHedgingQuantity should match")
				assert.Equal(t, types.NewNumberFromFloat(0), r.List[0].Coin[x].TotalOrderIM, "TotalOrderIM should match")
				...
				assert.Equal(t, types.NewNumberFromFloat(0), r.List[0].Coin[x].TotalPositionMM, "TotalPositionMM should match")
				assert.Equal(t, types.NewNumberFromFloat(0), r.List[0].Coin[x].UnrealisedPNL, "UnrealisedPNL should match")
				assert.Equal(t, types.NewNumberFromFloat(71233.0214024), r.List[0].Coin[x].USDValue, "USDValue should match")
				assert.Equal(t, types.NewNumberFromFloat(2473.9), r.List[0].Coin[x].WalletBalance, "WalletBalance should match")
			case 2:
				assert.Equal(t, types.NewNumberFromFloat(0), r.List[0].Coin[x].AccruedInterest, "AccruedInterest should match")
				assert.Equal(t, types.NewNumberFromFloat(0), r.List[0].Coin[x].AvailableToBorrow, "AvailableToBorrow should match")
				assert.Equal(t, types.NewNumberFromFloat(935.1415), r.List[0].Coin[x].AvailableToWithdraw, "AvailableToWithdraw should match")
				assert.Equal(t, types.NewNumberFromFloat(0), r.List[0].Coin[x].Bonus, "Bonus should match")
				assert.Equal(t, types.NewNumberFromFloat(0), r.List[0].Coin[x].BorrowAmount, "BorrowAmount should match")
				assert.Equal(t, currency.USDT, r.List[0].Coin[x].Coin, "Coin should match")
				assert.True(t, r.List[0].Coin[x].CollateralSwitch, "CollateralSwitch should match")
				assert.Equal(t, types.NewNumberFromFloat(0), r.List[0].Coin[x].CumulativeRealisedPNL, "CumulativeRealisedPNL should match")
				assert.Equal(t, types.NewNumberFromFloat(935.1415), r.List[0].Coin[x].Equity, "Equity should match")
				assert.Equal(t, types.NewNumberFromFloat(0), r.List[0].Coin[x].Locked, "Locked should match")
				assert.True(t, r.List[0].Coin[x].MarginCollateral, "MarginCollateral should match")
				assert.Equal(t, types.NewNumberFromFloat(0), r.List[0].Coin[x].SpotHedgingQuantity, "SpotHedgingQuantity should match")
				assert.Equal(t, types.NewNumberFromFloat(0), r.List[0].Coin[x].TotalOrderIM, "TotalOrderIM should match")
				...
				assert.Equal(t, types.NewNumberFromFloat(0), r.List[0].Coin[x].TotalPositionMM, "TotalPositionMM should match")
				assert.Equal(t, types.NewNumberFromFloat(0), r.List[0].Coin[x].UnrealisedPNL, "UnrealisedPNL should match")
				assert.Equal(t, types.NewNumberFromFloat(935.23875471), r.List[0].Coin[x].USDValue, "USDValue should match")
				assert.Equal(t, types.NewNumberFromFloat(935.1415), r.List[0].Coin[x].WalletBalance, "WalletBalance should match")
			}
		}
	}
}
...
func TestSetMMP(t *testing.T) {
	...
	err = b.SetMMP(context.Background(), &MMPRequestParam{
		...
		TimeWindowMS:       5000,
		FrozenPeriod:       100000,
		TradeQuantityLimit: types.NewNumberFromFloat(50),
		DeltaLimit:         types.NewNumberFromFloat(20),
	})
	if err != nil {
		t.Error(err)
	}
...
func TestCreateInternalTransfer(t *testing.T) {
	...
	_, err = b.CreateInternalTransfer(context.Background(), &TransferParams{
		TransferID: transferID,
		Coin:       currency.BTC,
		Amount:     types.NewNumberFromFloat(123.456),
	})
	if !errors.Is(err, errMissingAccountType) {
		t.Fatalf("expected %v, got %v", errMissingAccountType, err)
	}
	_, err = b.CreateInternalTransfer(context.Background(), &TransferParams{
		TransferID: transferID,
		Coin:       currency.BTC, Amount: types.NewNumberFromFloat(123.456),
	})
	if !errors.Is(err, errMissingAccountType) {
		t.Fatalf("expected %v, got %v", errMissingAccountType, err)
	}
	_, err = b.CreateInternalTransfer(context.Background(), &TransferParams{
		TransferID: transferID,
		Coin:       currency.BTC, Amount: types.NewNumberFromFloat(123.456), FromAccountType: "UNIFIED",
	})
	if !errors.Is(err, errMissingAccountType) {
		t.Fatalf("expected %v, got %v", errMissingAccountType, err)
	}
