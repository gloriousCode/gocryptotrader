...
func (e *Exchange) processIncrementalTicker(ctx context.Context, respRaw []byte, channels []string) error {
	...
	if a == asset.Options {
		...
		option = &exchangeoptions.Option{
			...
			Bid:               value(state.BestBidPrice),
			Ask:               value(state.BestAskPrice),
			BidSize:           value(state.BestBidAmount),
			AskSize:           value(state.BestAskAmount),
			MarkPrice:         value(state.MarkPrice),
			IndexPrice:        value(state.IndexPrice),
			UnderlyingPrice:   value(state.UnderlyingPrice),
			LastTradePrice:    value(state.LastPrice),
			OpenInterest:      value(state.OpenInterest),
			Volume24h:         value(state.Stats.Volume),
		}
	}
	e.incrementalTickersMtx.Unlock()
	...
	if option != nil {
		return e.Websocket.DataHandler.Send(ctx, option)
	}
	return nil
}
