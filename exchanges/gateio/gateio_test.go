...
func TestFuturesDataHandler(t *testing.T) {
	...
	assert.NotEmpty(t, e.Websocket.DataHandler.C, "futures fixture should emit messages")
...
func TestFuturesPositionCapturedPayload(t *testing.T) {
	...
	rawMessage := <-e.Websocket.DataHandler.C
	require.IsType(t, &WsFuturesPositionsEvent{}, rawMessage.Data, "captured payload must retain the raw futures position event")
	...
	rawMessage = <-e.Websocket.DataHandler.C
	require.IsType(t, &WsFuturesPositionsEvent{}, rawMessage.Data, "classic cross-margin payload must retain the raw position event")
	message = <-e.Websocket.DataHandler.C
	positions, ok = message.Data.([]futures.Position)
	...
	rawMessage = <-e.Websocket.DataHandler.C
	require.IsType(t, &WsFuturesPositionsEvent{}, rawMessage.Data, "zero-size payload must retain the raw position event")
	message = <-e.Websocket.DataHandler.C
	positions, ok = message.Data.([]futures.Position)
	...
	for _, tt := range []struct {
		...
		t.Run(tt.mode, func(t *testing.T) {
			...
			rawMessage := <-ex.Websocket.DataHandler.C
			require.IsType(t, &WsFuturesPositionsEvent{}, rawMessage.Data, "dual-mode payload must retain the raw position event")
	...
	t.Run("delivery collateral", func(t *testing.T) {
		...
		rawMessage := <-ex.Websocket.DataHandler.C
		require.IsType(t, &WsFuturesPositionsEvent{}, rawMessage.Data, "delivery payload must retain the raw position event")
		message := <-ex.Websocket.DataHandler.C
		positions, ok := message.Data.([]futures.Position)
...
func TestWsHandleFuturesDataLifecycleEvents(t *testing.T) {
	...
	for i := 0; ; i++ {
		...
		case *WsFuturesPositionsEvent:
			...
			require.Len(t, event.Result, 1)
			result := event.Result[0]
			require.Equal(t, int64(42), result.UpdateID)
			require.Equal(t, "cross", result.PositionMarginMode)
			require.Equal(t, 5.0, result.PositionLeverage.Float64())
...
func TestProcessFuturesOrdersPushData(t *testing.T) {
	...
	_, processed, err := e.processFuturesOrdersPushData([]byte(testCases[0].incoming), asset.CoinMarginedFutures)
	require.NoError(t, err)
	...
	_, processed, err = e.processFuturesOrdersPushData([]byte(incomingWithoutTIF), asset.CoinMarginedFutures)
...
func TestFuturesLiveCapturedPayloads(t *testing.T) {
	...
	t.Run("orders", func(t *testing.T) {
		...
		for _, tt := range []struct {
			...
			t.Run(tt.name, func(t *testing.T) {
				t.Parallel()
				_, processed, err := e.processFuturesOrdersPushData([]byte(tt.payload), asset.USDTMarginedFutures)
	...
	t.Run("user trade", func(t *testing.T) {
		...
		for _, tt := range []struct {
			...
			t.Run(tt.name, func(t *testing.T) {
				...
				require.NoError(t, testexch.Setup(ex), "Test instance Setup must not error")
				ex.Websocket.Fills.Setup(true, ex.Websocket.DataHandler)
				require.NoError(t, ex.processFuturesUserTrades(t.Context(), []byte(tt.payload), asset.USDTMarginedFutures),
					"processFuturesUserTrades must process captured live data")
				rawMessage := <-ex.Websocket.DataHandler.C
				require.IsType(t, &WsFuturesUserTradesEvent{}, rawMessage.Data, "captured trade must retain the raw futures trade event")
				message := <-ex.Websocket.DataHandler.C
	...
	t.Run("position", func(t *testing.T) {
		...
		rawMessage := <-ex.Websocket.DataHandler.C
		require.IsType(t, &WsFuturesPositionsEvent{}, rawMessage.Data, "captured position must retain the raw futures position event")
		message := <-ex.Websocket.DataHandler.C
		positions, ok := message.Data.([]futures.Position)
...
func TestProcessFinishedFuturesOrdersPushData(t *testing.T) {
	...
	t.Run("negative size", func(t *testing.T) {
		...
		_, processed, err := e.processFuturesOrdersPushData([]byte(incoming), asset.CoinMarginedFutures)
		require.NoError(t, err)
		require.Len(t, processed, 1, "futures order update must contain one order")
	...
	t.Run("market order", func(t *testing.T) {
		...
		_, processed, err := e.processFuturesOrdersPushData([]byte(incoming), asset.USDTMarginedFutures)
		require.NoError(t, err)
		require.Len(t, processed, 1, "futures order update must contain one order")
