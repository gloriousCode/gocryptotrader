...
func (e *Exchange) WsHandleFuturesData(ctx context.Context, conn websocket.Connection, respRaw []byte, a asset.Item) error {
	...
	case futuresUserTradesChannel:
		return e.processFuturesUserTrades(ctx, respRaw, a)
...
func (e *Exchange) generateFuturesPayload(ctx context.Context, event string, channelsToSubscribe subscription.List) ([]WsInput, error) {
	...
	for i := range channelsToSubscribe {
		...
		if privateChannel {
			...
			if channelsToSubscribe[i].Channel != futuresAutoOrdersChannel {
				user, ok := channelsToSubscribe[i].Params["user"].(string)
				placeholder, _ := channelsToSubscribe[i].Params[requiresUserPlaceholderParam].(bool)
				if (!ok || user == "") && !placeholder {
			...
			if channelsToSubscribe[i].Channel != futuresBalancesChannel {
				...
				} else if omitContract, _ := channelsToSubscribe[i].Params[omitContractParam].(bool); !omitContract {
					if contractOverride, ok := channelsToSubscribe[i].Params[contractPayloadOverrideParam].(string); ok {
						params = append(params, contractOverride)
					} else {
...
func (e *Exchange) processFuturesUserTrades(ctx context.Context, data []byte, assetType asset.Item) error {
...
func (e *Exchange) processPositionCloseData(ctx context.Context, data []byte, a asset.Item) error {
	raw := new(WsFuturesPositionClosesEvent)
	...
	if a == asset.Options {
		return e.Websocket.DataHandler.Send(ctx, raw)
	}
...
func (e *Exchange) processFuturesPositionsNotification(ctx context.Context, data []byte, a asset.Item) error {
	raw := new(WsFuturesPositionsEvent)
	if err := json.Unmarshal(data, raw); err != nil {
		return err
	}
	if err := e.Websocket.DataHandler.Send(ctx, raw); err != nil {
		return err
	}
	resp := struct {
		Time    types.Time          `json:"time"`
