...
func (k *Kraken) CalculatePNL(ctx context.Context, pnl *order.PNLCalculatorRequest) (*order.PNLResult, error) {
	if pnl == nil {
		return nil, fmt.Errorf("%v %w", k.Name, order.ErrNilPNLCalculator)
	}
	...
	if pnl.CalculateOffline {
		...
		if err != nil {
			return nil, fmt.Errorf("%s %s %w", k.Name, creds.SubAccount, err)
		}
	}

	ep := pnl.EntryPrice.InexactFloat64()
	info, err := k.GetFuturesAccountData(ctx)
	if err != nil {
		return nil, err
	}
	if info.Liquidating || info.Collateral <= 0 {
		result.IsLiquidated = true
		return result, fmt.Errorf("%s %s %w", k.Name, creds.SubAccount, order.ErrPositionLiquidated)
	}
...
func (k *Kraken) ScaleCollateral(ctx context.Context, calc *order.CollateralCalculator) (*order.CollateralByCurrency, error) {
	if calc.CalculateOffline {
		...
		if calc.USDPrice.IsZero() {
			return nil, fmt.Errorf("%s %s %w to scale collateral", k.Name, calc.CollateralCurrency, order.ErrUSDValueRequired)
		}
		if calc.FreeCollateral.IsZero() && calc.LockedCollateral.IsZero() {
			return result, nil
		}
		collateralWeight, ok := k.collateralWeight[calc.CollateralCurrency.Item]
		if !ok {
			return nil, fmt.Errorf("%s %s %w", k.Name, calc.CollateralCurrency, errCollateralCurrencyNotFound)
		}
		if calc.FreeCollateral.IsPositive() {
			if collateralWeight.InitialMarginFractionFactor == 0 {
				return nil, fmt.Errorf("%s %s %w", k.Name, calc.CollateralCurrency, errCollateralInitialMarginFractionMissing)
			}
	...
	resp, err := k.calculateTotalCollateralOnline(ctx,
	...
	if len(resp.BreakdownByCurrency) == 0 {
		return nil, fmt.Errorf("%v %v %w", k.Name, calc.CollateralCurrency, errCollateralCurrencyNotFound)
	}
...
func (k *Kraken) CalculateTotalCollateral(ctx context.Context, calc *order.TotalCollateralCalculator) (*order.TotalCollateralResponse, error) {
	if calc == nil {
		return nil, fmt.Errorf("%v CalculateTotalCollateral %w", k.Name, common.ErrNilPointer)
	}
	var pos []PositionData
	var err error
	if calc.FetchPositions {
		pos, err = k.GetPositions(ctx, true)
		if err != nil {
			return nil, fmt.Errorf("%v CalculateTotalCollateral GetPositions %w", k.Name, err)
		}
	}
	if !calc.CalculateOffline {
		return k.calculateTotalCollateralOnline(ctx, calc, pos)
	}
	...
	for i := range calc.CollateralAssets {
		...
		collateralByCurrency, err = k.ScaleCollateral(ctx, &calc.CollateralAssets[i])
		if err != nil {
...
func (k *Kraken) calculateTotalCollateralOnline(ctx context.Context, calc *order.TotalCollateralCalculator, pos []PositionData) (*order.TotalCollateralResponse, error) {
	if calc == nil {
		return nil, fmt.Errorf("%v CalculateTotalCollateral %w", k.Name, common.ErrNilPointer)
	}
	if len(calc.CollateralAssets) == 0 {
		return nil, fmt.Errorf("%v calculateTotalCollateralOnline %w, no currencies supplied", k.Name, errCollateralCurrencyNotFound)
	}
	if calc.CalculateOffline {
		return nil, fmt.Errorf("%v calculateTotalCollateralOnline %w", k.Name, order.ErrOfflineCalculationSet)
	}
...
func (k *Kraken) GetFuturesPositions(ctx context.Context, request *order.PositionsRequest) ([]order.PositionDetails, error) {
	return nil, nil
}
