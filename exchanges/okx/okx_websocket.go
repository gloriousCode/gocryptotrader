...
func (ok *Okx) WsHandleData(respRaw []byte) error {
	...
	if resp.ID != "" {
		if ok.Websocket.Match.IncomingWithData(resp.ID, respRaw) {
			return nil
		}
	}
	if resp.Event == operationLogin {
		if resp.Code == "0" {
			ok.Websocket.SetCanUseAuthenticatedEndpoints(true)
			return nil
		}
	}
	if resp.Event == "error" && (resp.Code == "60022" || resp.Code == "60009") {
		ok.Websocket.SetCanUseAuthenticatedEndpoints(false)
		return fmt.Errorf("authentication failed with error: %v", ErrorCodes[resp.Code])
	}
	if len(resp.Data) == 0 {
		return nil
	}
...
func (ok *Okx) wsProcessIndexCandles(intermediate *wsIncomingData) error {
	...
	for i := range response.Data {
		...
		k.OpenPrice, err = strconv.ParseFloat(candles[1], 64)
		if err != nil {
			return err
		}
		k.HighPrice, err = strconv.ParseFloat(candles[2], 64)
		if err != nil {
			return err
		}
		k.LowPrice, err = strconv.ParseFloat(candles[3], 64)
		if err != nil {
			return err
		}
		k.ClosePrice, err = strconv.ParseFloat(candles[4], 64)
		if err != nil {
			return err
		}
		ok.Websocket.DataHandler <- k
	}
	return nil
}
...
func (ok *Okx) WsPlaceOrder(arg *PlaceOrderRequestParam) (*OrderData, error) {
	...
	respRaw, err := ok.Websocket.AuthConn.SendMessageReturnResponse(randomID, input)
	if err != nil {
		return nil, err
	}
	...
	if len(resp.Data) == 1 {
		if resp.Data[0].OrderID == "" {
			return nil, fmt.Errorf("could not place order %v %v %v %v %v-%v", arg.Currency, arg.TradeMode, arg.Side, arg.Amount, resp.Data[0].SCode, resp.Data[0].SMessage)
		}
...
func (ok *Okx) WsCancelOrder(arg CancelOrderRequestParam) (*OrderData, error) {
	...
	return nil, fmt.Errorf("error code: %s message: %v", resp.Code, ErrorCodes[resp.Code])
}
...
func (ok *Okx) WsChannelSubscription(operation, channel string, assetType asset.Item, pair currency.Pair, tooglers ...bool) error {
	...
	ok.Websocket.AddSuccessfulSubscriptions(stream.ChannelSubscription{
		Channel:  channel,
		Currency: pair,
		Asset:    assetType,
	})
