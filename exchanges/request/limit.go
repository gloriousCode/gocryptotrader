...
func (r *RateLimiterWithWeight) RateLimit(ctx context.Context) error {
	...
	r.m.Lock()
	tn := time.Now()
	reserved, finalDelay := r.reserveLocked(tn, weight)
...
func (r *RateLimiterWithWeight) reserveLocked(at time.Time, weight Weight) ([]*rate.Reservation, time.Duration) {
...
func (b *rateLimitBarrier) admitLocked() error {
	...
	for _, participant := range b.participants {
		...
		reserved, delay := participant.limiter.reserveLocked(at, participant.weight)
