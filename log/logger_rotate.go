...
// SetFilePath sets the internal filepath of the logger
// to save log files to
func SetFilePath(fp string) error {
	if fp == "" {
		return errors.New(filePathEmpty)
	}
	rwm.Lock()
	filePath = fp
	rwm.Unlock()
...
// FilePath returns the internal filepath of the logger
